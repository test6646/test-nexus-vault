...
export function calculateTotalClosed(event: EventFinancials): number {
  if (event.event_closing_balances && event.event_closing_balances.length > 0) {
    return event.event_closing_balances.reduce((sum, closing) => sum + (closing.closing_amount || 0), 0);
  }
  return 0;
}
