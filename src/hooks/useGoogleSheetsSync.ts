...
export const useGoogleSheetsSync = () => {
  ...
  const autoSyncItem = async (itemType: 'client' | 'event' | 'task' | 'expense' | 'staff' | 'freelancer', itemId: string) => {
    if (!currentFirmId) {
      return;
    }
  ...
  const syncItemToSheets = async (itemType: 'client' | 'event' | 'task' | 'expense' | 'staff' | 'freelancer', itemId: string) => {
    if (!currentFirmId) {
      return;
    }
