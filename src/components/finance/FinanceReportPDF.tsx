...
const FinanceReportDocument: React.FC<FinanceReportProps> = ({ stats, timeRange, firmData }) => {
    ...
    <Document>
      <Page size="A4" style={styles.page}>
        ...
        <View style={styles.header}>
          ...
          <View style={styles.companyInfo}>
            ...
            {firmData?.header_left_content?.split('\n').map((line, index) => (
              <Text key={index} style={styles.tagline}>{line}</Text>
            ))}
            <Text style={[styles.tagline, { fontWeight: 600, marginTop: 2 }]}>{firmDescription}</Text>
          </View>
        </View>
        ...
        <Text style={styles.footer}>
          {firmData?.footer_content || `${firmName} | Professional Photography & Videography Services`}{'\n'}
...
const generateFinanceReportPDF = async (stats: any, timeRange: string, firmData?: { name: string; description?: string; logo_url?: string; header_left_content?: string; footer_content?: string; }) => {
  const blob = await pdf(<FinanceReportDocument stats={stats} timeRange={timeRange} firmData={firmData} />).toBlob();
