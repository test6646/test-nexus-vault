...
// Helper function to determine payment status based on ALL payments and closing balances for the event
const getPaymentStatus = (payment: Payment & { event?: Event }): 'Paid' | 'Pending' | 'Partial' | 'Overdue' => {
  ...
  // Calculate total of ALL payments for this event (including advance amount)
  const allPayments = payment.event.payments || [];
  const totalPaidAmount = allPayments.reduce((sum, p) => sum + (p.amount || 0), 0) + (payment.event.advance_amount || 0);
  
  // Calculate total closing amounts
  const closingBalances = (payment.event as any)?.event_closing_balances || [];
  const totalClosingAmount = closingBalances.reduce((sum: number, closing: any) => sum + (closing.closing_amount || 0), 0);
  
  // Calculate total covered amount (payments + closings)
  const totalCoveredAmount = totalPaidAmount + totalClosingAmount;
  
  if (totalCoveredAmount >= eventTotalAmount) {
    return 'Paid';
  } else if (totalCoveredAmount > 0 && totalCoveredAmount < eventTotalAmount) {
    return 'Partial';
  } else {
            ...
            {payment.event && (
              <>
                ...
                <View style={styles.detailRow}>
                  ...
                  <Text style={styles.detailValue}>₹{((payment.event.payments?.reduce((sum, p) => sum + (p.amount || 0), 0) || 0) + (payment.event.advance_amount || 0)).toLocaleString()}</Text>
                </View>
                <View style={styles.detailRow}>
                  <Text style={styles.detailLabel}>Balance Due:</Text>
                  <Text style={styles.detailValue}>₹{Math.max(0, (payment.event.total_amount || 0) - ((payment.event.payments?.reduce((sum, p) => sum + (p.amount || 0), 0) || 0) + (payment.event.advance_amount || 0)) - ((payment.event as any).event_closing_balances?.reduce((sum: number, closing: any) => sum + (closing.closing_amount || 0), 0) || 0)).toLocaleString()}</Text>
                </View>
              </>
            )}
          </View>
...
export const generatePaymentInvoicePDF = async (payment: Payment & { event?: Event }, firmData?: any, downloadOnly = true) => {
  try {
    ...
    const allPayments = payment.event?.payments || [];
    const totalAlreadyPaid = allPayments.reduce((sum, p) => sum + (p.amount || 0), 0) + (payment.event?.advance_amount || 0);
    
    // Get closing balances for this event
    const closingBalances = (payment.event as any)?.event_closing_balances || [];
    const totalClosingAmount = closingBalances.reduce((sum: number, closing: any) => sum + (closing.closing_amount || 0), 0);
