...
const CustomizableQuotationBuilder = ({ formData, onBack, onComplete, editingQuotation }: CustomizableQuotationBuilderProps) => {
  ...
  const postProductionCost = postProductionOptions[selectedPostProductionPackage as keyof typeof postProductionOptions];
    ...
    <div className="space-y-6">
      ...
      <div className={`grid ${isMobile ? 'grid-cols-1' : 'grid-cols-1 lg:grid-cols-3'} gap-4`}>
          ...
          <CardContent className="space-y-3 px-3 pb-3">
            ...
            <div className="space-y-2">
              ...
              <div className="grid grid-cols-3 gap-2">
                {Object.entries(postProductionOptions).map(([key, value]) => (
                  <Button
                    key={key}
                    variant={selectedPostProductionPackage === key ? "default" : "outline"}
                    size="sm"
                    onClick={() => setSelectedPostProductionPackage(key)}
        ...
        <Card className="h-fit rounded-3xl border-2 border-primary/30">
          ...
          <CardContent className="space-y-3 px-3 pb-3">
            {addOns.map((addOn) => (
              <div key={addOn.id} className="flex items-center justify-between p-3 border rounded-2xl hover:bg-primary/5 transition-colors">
