...
const ClientTableManagement = () => {
  ...
  const handleDelete = async (client: Client) => {
    setConfirmDialog({
      ...
      onConfirm: async () => {
        ...
        setSubmitting(true);
        try {
          // First sync delete to Google Sheets while client data still exists using the DELETE function
          try {
            const { error: deleteError } = await supabase.functions.invoke('delete-item-from-google', {
              body: {
                itemType: 'client',
                itemId: client.id,
            ...
            if (deleteError) {
              console.warn('Google Sheets delete failed:', deleteError, 'but continuing with database deletion');
            } else {
              console.log('✅ Successfully deleted client from Google Sheets');
            }
          } catch (sheetError) {
            console.error('Google Sheets delete error:', sheetError);
  ...
  // Show all clients without filtering
  const clientsToShow = clients;
