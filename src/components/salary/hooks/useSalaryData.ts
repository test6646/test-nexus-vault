    ...
    try {
      ...
      const salaryData: StaffSalaryData[] = (staffMembers || []).map(staff => {
        ...
        const assignmentEarnings = staffAssignmentRates
          .reduce((sum, assignment) => sum + ((Number(assignment.rate) || 0) * (Number(assignment.quantity) || 0)), 0);
      ...
      // 🚀 OPTIMIZED: Calculate freelancer earnings more efficiently
      let freelancerTaskEarnings = 0;
      let freelancerAssignmentEarnings = 0;
      let freelancerPaidAmount = 0;

      if (freelancers && freelancers.length > 0) {
        // Calculate task earnings for freelancers - ALL tasks with amounts (not just completed)
        freelancerTaskEarnings = (freelancerTasks || [])
          .reduce((sum, task) => sum + (Number(task.amount) || 0), 0);

        // Calculate assignment earnings for freelancers
        const freelancerAssignments = eventAssignments?.filter(assignment => assignment.freelancer_id) || [];
        freelancerAssignmentEarnings = freelancerAssignments
          .reduce((sum, assignment) => sum + ((Number(assignment.rate) || 0) * (Number(assignment.quantity) || 0)), 0);

        // Calculate freelancer payments
        freelancerPaidAmount = (freelancerPayments || []).reduce((sum, payment) => sum + (Number(payment.amount) || 0), 0);
      }

      // 🚀 OPTIMIZED: Simplified stats calculation  
      const totalStaff = staffMembers?.length || 0;
      const totalFreelancers = freelancers?.length || 0;
      
      // Calculate total earnings including ALL tasks and assignments for both staff and freelancers
      const staffTotalEarnings = salaryData.reduce((sum, staff) => sum + staff.total_earnings, 0);
      const freelancerTotalEarnings = freelancerTaskEarnings + freelancerAssignmentEarnings;
      const totalEarnings = staffTotalEarnings + freelancerTotalEarnings;
      
      // For display purposes - completed task earnings only
      const totalCompletedTaskEarnings = salaryData.reduce((sum, staff) => sum + staff.task_earnings, 0) + (freelancerTasks || []).filter(task => task.status === 'Completed').reduce((sum, task) => sum + (Number(task.amount) || 0), 0);
      const totalAssignmentEarnings = salaryData.reduce((sum, staff) => sum + staff.assignment_earnings, 0) + freelancerAssignmentEarnings;
      
      const totalPaid = salaryData.reduce((sum, staff) => sum + staff.paid_amount, 0) + freelancerPaidAmount;
      const totalPending = Math.max(0, totalEarnings - totalPaid);

      const totalPeople = totalStaff + totalFreelancers;
      const avgPerPerson = totalPeople > 0 ? totalEarnings / totalPeople : 0;

      setTotalStats({
        totalStaff,
        totalFreelancers,
        taskPaymentsTotal: totalCompletedTaskEarnings,
