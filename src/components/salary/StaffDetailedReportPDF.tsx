...
export const generateStaffDetailedReportPDF = async (staff: StaffData) => {
  // ✅ FIX: Get current user's firm data using current_firm_id from auth context
  let firmData = null;
  try {
    // Get firm ID from localStorage (same pattern as other PDFs)
    const currentFirmId = localStorage.getItem('selectedFirmId');
