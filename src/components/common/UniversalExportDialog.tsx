...
const UniversalExportDialog: React.FC<UniversalExportDialogProps> = ({ 
    ...
    return data.filter(item => {
      switch (filterType) {
        ...
        case 'staff':
          ...
          return item.assigned_to === selectedValue || item.id === selectedValue || item.full_name === selectedValue;
        case 'freelancers':
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      await config.exportFunction(filteredData, filterType, actualSelectedValue || getFilterDisplayValue());
      toast({
