...
import { DuplicateEventDialog } from '@/components/ui/duplicate-event-dialog';
import { FinancialValidationDialog } from '@/components/ui/financial-validation-dialog';
...
const CleanEventFormDialog = ({ open, onOpenChange, event, editingEvent, onSuccess }: CleanEventFormDialogProps) => {
  ...
  const [duplicateEventInfo, setDuplicateEventInfo] = useState<{
    eventType: string;
  ...
  const validateForm = (): boolean => {
    ...
    // Validate total bill vs advance amount
    if (formData.total_amount === 0 && extendedData.advance_amount > 0) {
      setFinancialError({
        title: "Invalid Financial Setup",
        description: `You cannot set total bill to ₹0 when advance amount is ₹${extendedData.advance_amount.toLocaleString()}. Please adjust the amounts.`
      });
      setShowFinancialDialog(true);
      return false;
    }

    // Validate advance not exceeding total
    if (extendedData.advance_amount > formData.total_amount) {
      setFinancialError({
        title: "Invalid Advance Amount",
        description: `Advance amount (₹${extendedData.advance_amount.toLocaleString()}) cannot exceed total bill (₹${formData.total_amount.toLocaleString()}).`
      });
      setShowFinancialDialog(true);
      return false;
    }
  ...
  const saveEvent = async (eventDataOverride?: any) => {
    try {
      const sanitizedFormData = sanitizeUuidFields(formData, [
        'client_id'
      ]);

      const eventData = eventDataOverride || {
        ...sanitizedFormData,
        ...extendedData,
      ...
      if (currentEvent) {
        ...
        // Check for duplicate event type for same client before inserting
        if (formData.client_id && formData.event_type) {
          ...
          if (existingEvents && existingEvents.length > 0) {
            ...
            setDuplicateEventInfo({
              ...
              existingEventTitle: existingEvents[0].title
            });
      ...
      // Send event confirmation notification for new events
      if (!currentEvent && formData.client_id) {
        try {
          ...
          if (client?.phone) {
            ...
            console.log('Event confirmation notification sent');
          }
        } catch (error) {
          console.error('Failed to send event confirmation:', error);
        }
      }
        ...
        const runBackgroundTasks = async () => {
          try {
            ...
            await Promise.allSettled([
              ...
              // Use centralized Google Sheets sync service
              import('@/services/googleSheetsSync').then(({ syncEventInBackground }) => {
                syncEventInBackground(result.id, result.firm_id, currentEvent ? 'update' : 'create');
              }),
  ...
  const sendEventNotifications = async (eventId: string, eventTitle: string) => {
    try {
      ...
      for (const assignment of assignments) {
        ...
        if (!phone) {
          console.log(`Skipping ${name} - no phone available`);
          continue;
        }
        ...
        try {
          ...
          if (error) {
            console.error(`Failed to send WhatsApp notification to ${name}:`, error);
          } else {
            console.log(`✅ WhatsApp notification sent to ${name} for ${assignment.role} role`);
          }
        } catch (whatsappError) {
          console.error(`⚠️ WhatsApp notification error for ${name}:`, whatsappError);
        }
      }
    } catch (error) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
               ...
               <div className="space-y-2">
                ...
                <div className="space-y-2">
                  ...
                  <div className="relative">
                    ...
                    />
                    {formData.venue && venueSuggestions.length > 0 && (
      ...
      <DuplicateEventDialog
        ...
        eventType={duplicateEventInfo?.eventType || ''}
        existingEventTitle={duplicateEventInfo?.existingEventTitle || ''}
        onConfirm={handleDuplicateConfirm}
        onCancel={handleDuplicateCancel}
      />

      <FinancialValidationDialog
        open={showFinancialDialog}
        onOpenChange={setShowFinancialDialog}
        title={financialError?.title || ''}
        description={financialError?.description || ''}
        onOk={handleFinancialDialogOk}
      />
    </Dialog>
  );
};
