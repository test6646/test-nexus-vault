  ...
  const populateFormWithCompleteData = async () => {
    ...
    try {
      ...
      setExtendedData({
        ...
        same_day_editor: completeEvent.same_day_editor || quotationDetails?.sameDayEditing || false,
      });
        ...
        const runBackgroundTasks = async () => {
          try {
            ...
            const backgroundTasks = [
              ...
              // Use new background sync service for Google Sheets
              import('@/services/googleSheetsSync').then(({ syncEventInBackground }) => 
                syncEventInBackground(result.id, profile?.current_firm_id || '', currentEvent ? 'update' : 'create')
              ),
...
_Thank you for being part of Prit Photo_ ✨`;
  ...
  const saveStaffAssignments = async (eventId: string) => {
    try {
      ...
      if ((quotationHasSameDayEditing || extendedData.same_day_editor) && sameDayEditors.some(id => id && id.trim() !== '')) {
