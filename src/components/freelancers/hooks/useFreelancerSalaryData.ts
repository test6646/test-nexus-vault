...
export const useFreelancerSalaryData = () => {
  ...
  const loadFreelancerSalaryData = async () => {
    ...
    try {
      ...
      // Get event details separately to avoid relation issues
      const eventIds = eventAssignments?.map(a => a.event_id).filter(Boolean) || [];
      ...
      const salaryData: FreelancerSalaryData[] = (freelancers as Freelancer[]).map(freelancer => {
        ...
        // Get event assignments for this freelancer
        const freelancerAssignments = eventAssignments?.filter(assignment => assignment.freelancer_id === freelancer.id) || [];
        ...
        const assignmentEarnings = freelancerAssignments
        ...
        return {
          ...
          total_assignments: freelancerAssignments.length,
          ...
          eventAssignments: freelancerAssignments.map(assignment => {
  ...
  useEffect(() => {
    ...
    console.log('Setting up real-time subscriptions for freelancer salary data...');
    
    // Set up real-time subscriptions for data changes
    const channel = supabase
      .channel(`freelancer-salary-data-${profile.current_firm_id}`)
      .on(
        'postgres_changes',
        {
          event: '*',
        ...
        (payload) => {
          console.log('REAL-TIME: Task updated:', payload);
      ...
      .on(
        ...
        (payload) => {
          console.log('REAL-TIME: Event assignment rates updated:', payload);
      ...
      .on(
        ...
        (payload) => {
          console.log('REAL-TIME: Freelancer payment updated:', payload);
      ...
      .on(
        ...
        (payload) => {
          console.log('REAL-TIME: Freelancer updated:', payload);
      ...
      .on(
        ...
        (payload) => {
          console.log('REAL-TIME: Event staff assignment updated:', payload);
          // Immediate UI update for assignment changes
          loadFreelancerSalaryData();
        }
      )
      .subscribe((status) => {
        console.log('Freelancer salary real-time subscription status:', status);
      });

    return () => {
      console.log('Cleaning up freelancer salary real-time subscriptions...');
