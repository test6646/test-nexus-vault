...
serve(async (req) => {
  ...
  try {
    ...
    // Format header and footer content with actual firm data
    const headerLeftContent = `Contact: ${contactPhone || 'YOUR_FIRM_CONTACT'}\nEmail: ${contactEmail || 'YOUR_FIRM_CONTACT_EMAIL'}`;
    const footerContent = `${firmName} | Contact: ${contactPhone || 'YOUR_FIRM_CONTACT'} | Email: ${contactEmail || 'YOUR_FIRM_CONTACT_EMAIL'}\n${description || 'YOUR_FIRM_TAGLINE'} | #aJourneyOfLoveBy${firmName.replace(/\s+/g, '')}`;
    ...
    try {
      const { data: firm, error: firmError } = await supabase
        ...
        .insert({
          ...
          header_left_content: headerLeftContent,
      ...
      try {
        const { error: waSessionError } = await supabase
          ...
          .upsert({
            ...
            contact_info: headerLeftContent,
            footer_signature: footerContent,
            status: 'disconnected',
