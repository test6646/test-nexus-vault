...
async function addEventToSheet(supabase: any, accessToken: string, spreadsheetId: string, eventId: string) {
  ...
  const { data: staffAssignments } = await supabase
    ...
    .select(`
      *,
      profiles!event_staff_assignments_staff_id_fkey(id, full_name)
    `)
    .eq('event_id', eventId);
  ...
  if (staffAssignments && staffAssignments.length > 0) {
    staffAssignments.forEach(assignment => {
      ...
      const staffName = assignment.profiles?.full_name || '';
      
      console.log(`👤 Staff assignment - Day ${day}: ${assignment.role} -> ${staffName}`);
